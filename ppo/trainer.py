...
class Trainer:
    ...
    def __init__(self, ppo_model, sess, info, is_continuous, use_observations, use_states, training):
        ...
        self.training_buffer = hry.vectorize_history(hry.empty_local_history({}))
    ...
    def process_experiences(self, info, time_horizon, gamma, lambd):
        ...
        for l in range(len(info.agents)):
            if (info.local_done[l] or len(self.history_dict[info.agents[l]]['actions']) > time_horizon) and len(
                ...
                history = hry.vectorize_history(self.history_dict[info.agents[l]])
                history['advantages'] = hry.get_gae(rewards=history['rewards'],
                ...
                if len(self.training_buffer['actions']) > 0:
                    hry.append_history(global_buffer=self.training_buffer, local_buffer=history)
                else:
                    hry.set_history(global_buffer=self.training_buffer, local_buffer=history)
                self.history_dict[info.agents[l]] = hry.empty_local_history(self.history_dict[info.agents[l]])
                if info.local_done[l]:
    ...
    def reset_buffers(self, brain_info=None, total=False):
        ...
        if not total:
            for key in self.history_dict:
                self.history_dict[key] = hry.empty_local_history(self.history_dict[key])
        else:
            self.history_dict = hry.empty_all_history(agent_info=brain_info)
    ...
    def update_model(self, batch_size, num_epoch):
        ...
        for k in range(num_epoch):
            training_buffer = hry.shuffle_buffer(self.training_buffer)
